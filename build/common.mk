...
MYTEST_PATH                     ?= $(ROOT)/my_test
SEC_STORAGE_TEST_PATH           ?= $(ROOT)/secStorTest
...
# my_test
################################################################################
MYTEST_COMMON_FLAGS ?= HOST_CROSS_COMPILE=$(CROSS_COMPILE_NS_USER)\
        TA_CROSS_COMPILE=$(CROSS_COMPILE_S_USER) \
        TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR) \
        TEEC_EXPORT=$(OPTEE_CLIENT_EXPORT)

.PHONY: mytest-common
mytest-common: optee-os optee-client
	$(MAKE) -C $(MYTEST_PATH) $(MYTEST_COMMON_FLAGS)

MYTEST_CLEAN_COMMON_FLAGS ?= TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR)

.PHONY: mytest-clean-common
mytest-clean-common:
	$(MAKE) -C $(MYTEST_PATH) $(MYTEST_CLEAN_COMMON_FLAGS) clean

################################################################################
# secure storage test
################################################################################
SEC_STORAGE_TEST_COMMON_FLAGS ?= HOST_CROSS_COMPILE=$(CROSS_COMPILE_NS_USER)\
        TA_CROSS_COMPILE=$(CROSS_COMPILE_S_USER) \
        TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR) \
        TEEC_EXPORT=$(OPTEE_CLIENT_EXPORT)
...
secStorTest-common: optee-os optee-client
	$(MAKE) -C $(SEC_STORAGE_TEST_PATH) $(SEC_STORAGE_TEST_COMMON_FLAGS)

SEC_STORAGE_TEST_CLEAN_COMMON_FLAGS ?= TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR)

.PHONY: secStorTest-clean-common
secStorTest-clean-common:
	$(MAKE) -C $(SEC_STORAGE_TEST_PATH) $(SEC_STORAGE_TEST_CLEAN_COMMON_FLAGS) clean
...
filelist-tee-common: optee-client xtest optee-examples mytest secStorTest save_key
	...
	@if [ -e $(MYTEST_PATH)/host/my_test ]; then \
		echo "file /bin/my_test" \
		     "$(MYTEST_PATH)/host/my_test 755 0 0" >> $(fl); \
                echo "file /lib/optee_armtz/9269fadd-99d5-4afb-a1dc-ee3e9c61b04c.ta" \
		     "$(MYTEST_PATH)/ta/9269fadd-99d5-4afb-a1dc-ee3e9c61b04c.ta" \
		     "444 0 0"                            >> $(fl); \
        fi
